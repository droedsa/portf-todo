...
const TaskItem = ({selectedTask, priorities, openChangeModal, deleteTask, doneBtn}) => {
    return selectedTask.map(({categories, done, id, name, priority}) => {
        const color = priorities.find(({priority}) => priority === priority);
        ...
        >
            <div className="task" style={{backgroundColor: done ? 'grey' : null}}>
